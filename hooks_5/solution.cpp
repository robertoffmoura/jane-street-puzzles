...
const int n = 9;
int top[n] = {41, 8, 0, 0, 14, 0, 15, 0, 0};
int bottom[n] = {0, 9, 0, 17, 0, 15, 0, 35, 0};
int left[n] = {0, 0, 25, 0, 15, 0, 26, 0, 0};
int right[n] = {25, 0, 0, 0, 10, 0, 0, 0, 27};
// const int n = 4;
// int top[n] = {0, 5, 0, 4};
// int bottom[n] = {3, 0, 7, 0};
// int left[n] = {8, 0, 0, 0};
// int right[n] = {0, 0, 0, 13};
...
int getFirstConsecutiveBlockSum(std::function<int(int)> getElement) {
	int current = 0;
	for (int k = 0; k < n; k++) {
...
bool isLinePartiallyOk(int constraint, std::function<int(int)> getElement) {
	...
	int sum = getFirstConsecutiveBlockSum(getElement);
