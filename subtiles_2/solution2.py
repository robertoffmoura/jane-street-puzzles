...
def fits(a,b): # returns whether a fits in b
	a = normalize(a)
	b = normalize(b)
	return any(fits_helper(new_a, b) for new_a in iter_orientations(a))
	...
	while len(q) > 0:
		...
		for i,j in q:
			...
			for di,dj in directions:
				if (i+di,j+dj) in shape_set:
					new_q.append((i+di,j+dj))
		q = new_q
	return len(seen) == len(shape)

def iter_orientations(shape):
	"""Yields all 8 possible orientations (4 rotations * 2 flips)."""
...
def valid_positions_in_board(shape,k):
	shape = normalize(shape)
	for shape_orientation in iter_orientations(shape):
		max_i = max(i for i,_ in shape_orientation)
		max_j = max(j for _,j in shape_orientation)
		for di in range(n - max_i):
			for dj in range(n - max_j):
				new_shape = set(translate(shape_orientation,di,dj))
				if (all(result[i][j] is None for (i,j) in new_shape)
					and all(grid[i][j] is None or grid[i][j] == k for (i,j) in new_shape)
					and all(cell in new_shape for cell in k_to_cells[k])):
					yield list(new_shape)

def add_to_board(shape,k):
	for i,j in shape:
		result[i][j] = k

def remove_from_board(shape):
	for i,j in shape:
		result[i][j] = None
...
def recurse2(k,shape_above):
	...
	if k == 0:
		...
		return
	shape = shape_above[:]
	for cell in shape_above:
		shape.remove(cell)
		if not is_connected(shape):
			shape.append(cell)
			continue
		for new_shape in valid_positions_in_board(shape,k):
			add_to_board(new_shape,k)
			recurse2(k-1,new_shape)
			remove_from_board(new_shape)
		shape.append(cell)

print("\n" * n)
recurse()

row_sums = [sum(solution[i][j] for j in range(n)) for i in range(n)]
print(f"min * max row sum = {min(row_sums) * max(row_sums)}")
